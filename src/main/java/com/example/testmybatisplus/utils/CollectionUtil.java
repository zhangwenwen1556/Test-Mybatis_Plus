...
public class CollectionUtil {
    ...
    public static <K, V> Map<K, List<V>> buildMultimap(List<V> objs, Class<K> keyClazz, Class<V> valClazz, String property) {
        if (isEmpty(objs)) {
     ...
     */
    @SuppressWarnings("unchecked")
    public static <K, V> Map<K, V> buildMap(List<V> objs, Class<K> keyClazz, Class<V> valClazz, String property) {
        if (isEmpty(objs)) {
            return Maps.newHashMapWithExpectedSize(0);
        }
     ...
     * @param splitStr   分隔符号 集合{User[name=a], User[name=b], User[name=c]} field名name 分隔符号逗号 ===> a,b,c
     */
    ...
    /**
     * 将来源数据列表的元素赋值给目标数据列表的指定属性. 参考{@link org.springframework.beans.BeanUtils#copyProperties(Object, Object)} 举例: sourceList为userList,
     * targetList为orderList, 那么joinEntry为<User.id, Order.uid> fields为[Order.User, Order.XXXUser]
     *
     ...
     */
    public static void copyProperties(List<?> sourceList, List<?> targetList, Map.Entry<String, String> joinEntry, String field) {
     ...
     */
    public static void copyProperties(List<?> sourceList, List<?> targetList, Map.Entry<String, String> joinEntry, List<String> fields) {
    ...
    /**
     * 将来源数据列表的指定属性赋值给目标数据列表的指定属性. 参考{@link org.springframework.beans.BeanUtils#copyProperties(Object, Object)} 举例: sourceList为userList,
     * targetList为orderList, 那么joinEntry为<User.id, Order.uid> mapping为[{user.name, order.userName}, {user.age, order.userAge}, ...]
     *
     * @param sourceList 来源数据列表
     * @param targetList 目标数据列表
     * @param joinEntry  聚合Entry<来源数据的key, 目标数据的key>
     ...
     */
    public static void copyProperties(List<?> sourceList, List<?> targetList, Map.Entry<String, String> joinEntry, Map<String, String> mapping) {
        // 参数校验部分
    ...
    /**
     * 将来源数据Map的指定属性赋值给目标数据列表的指定属性. 参考{@link org.springframework.beans.BeanUtils#copyProperties(Object, Object)} 举例: sourceMap为userMap,
     * targetList为orderList, 那么joinEntry为<User.id, Order.uid> mapping为[{user.name, order.userName}, {user.age, order.userAge}, ...]
     *
     ...
     * @param mapping    字段映射Map<来源数据的字段key, 目标数据的字段key>
     */
    private static void copyProperties(Map<?, ?> sourceMap, List<?> targetList, Map.Entry<String, String> joinEntry, Map<String, String> mapping) {
        // 参数校验部分
